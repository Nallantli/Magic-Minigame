function drawBattleField(battleState, iterator) {
	// battleData, selectedCards, selectedVictims, playerIndex) {
	let inputData = {};
	const {
		turnState: { battleData, selectedCards, selectedVictims, aura },
		playerIndex,
	} = battleState;

	if (aura) {
		spellSpriteDirectory[`${aura.id}.aura`].draw(ctx, 0, 0, scale(480), scale(270), {
			iIndex: iterator % spellSpriteDirectory[`${aura.id}.aura`].indices,
		});
	}

	const selectedPlayerSpell =
		selectedCards[playerIndex] === null ||
		selectedCards[playerIndex] === "PASS" ||
		battleData[playerIndex].hand?.[selectedCards[playerIndex]]?.id === undefined
			? null
	...
	for (let i = 0; i < 4; i++) {
		...
		if (
			selectedPlayerSpell === null ||
			selectedVictims[playerIndex].length > 0 ||
			selectedPlayerSpell.type === SPELL_TYPES.ENCHANTMENT
		) {
			ctx.globalAlpha = 1;
			...
			makeInteractable(
				...
				() => {},
				({ x, y, sizeX, sizeY }) =>
					selectedPlayerSpell !== null && sprites.PLACARD_160x65.draw(ctx, x, y, sizeX, sizeY),
				() => {
		...
		shields.forEach(({ value, id, element, elementTo }, j) => {
			makeInteractable(
				...
				({ x, y, renderCallback }) => {
					...
					const str =
						elementTo === undefined
							? `${value > 0 ? "+" : ""}${value}% Incoming`
							: `${element.toUpperCase()} to ${elementTo.toUpperCase()}`;
		...
		for (let j = 0; j < superVril; j++) {
			sprites.SUPER_VRIL_4x4.draw(
				ctx,
				scale(6),
				(i + 1) * scale(67) - j * scale(6) - scale(10),
				scale(4),
				scale(4),
				{ iIndex: ELEMENT_COLORS[entity.element] }
			);
		...
		for (let j = 0; j < vril; j++) {
			sprites.VRIL_4x4.draw(
				ctx,
				scale(6),
				(i + 1) * scale(67) - (j + superVril) * scale(6) - scale(10),
				scale(4),
				scale(4),
				{ iIndex: ELEMENT_COLORS[entity.element] }
			);
		...
		if (playerIndex < 4) {
			const selectedEntitySpell =
				selectedCards[i] === null ||
				selectedCards[i] === "PASS" ||
				battleData[i].hand?.[selectedCards[i]]?.id === undefined
					? null
					: getSpell(battleData[i].hand?.[selectedCards[i]]?.id);
			if (selectedEntitySpell) {
				getCardSprite(selectedEntitySpell).draw(
					ctx,
					scale(168),
					i * scale(67) + scale(10),
					scale(24),
				...
				if (battleData[i].hand[selectedCards[i]].enchantments) {
					sprites.ENCHANTMENT_BORDER_48x64.draw(
						ctx,
						scale(168),
						i * scale(67) + scale(10),
						scale(24),
				...
				if (selectedVictims[i].length > 0) {
					...
					if (index < 4) {
						numberText.draw(ctx, scale(170), i * scale(67) + scale(49), scale(8), scale(12), {
							iIndex: 0,
							text: selectedVictims[i].length === 1 ? String(index + 1) : "*",
						});
						sprites.VICTIM_ARROW_8x16.draw(
							ctx,
							scale(182),
							i * scale(67) + scale(46),
							scale(8),
						...
						numberText.draw(ctx, scale(182), i * scale(67) + scale(49), scale(8), scale(12), {
							iIndex: 0,
							text: selectedVictims[i].length === 1 ? String(index - 3) : "*",
						});
						sprites.VICTIM_ARROW_8x16.draw(
							ctx,
							scale(170),
	...
	for (let i = 4; i < 8; i++) {
		...
		if (
			...
			makeInteractable(
				...
				() => {},
				({ x, y, sizeX, sizeY }) =>
					selectedPlayerSpell !== null && sprites.PLACARD_RIGHT_160x65.draw(ctx, x, y, sizeX, sizeY),
				() => {
					if (selectedPlayerSpell !== null && canUseSpellOn(selectedPlayerSpell, playerIndex, i)) {
						inputData.selectedVictims = [i];
					}
		...
		shields.forEach(({ value, id, element, elementTo }, j) => {
			makeInteractable(
				scale(480 - 67) - j * scale(12),
				i_offset * scale(67) + scale(28),
				scale(10),
				scale(12),
				({ x, y, sizeX, sizeY }) => spellSpriteDirectory[id].draw(ctx, x, y, sizeX, sizeY),
				({ x, y, renderCallback }) => {
		...
		for (let j = 0; j < superVril; j++) {
			sprites.SUPER_VRIL_4x4.draw(
				ctx,
				scale(480 - 10),
				(i_offset + 1) * scale(67) - j * scale(6) - scale(10),
				scale(4),
				scale(4),
				{ iIndex: ELEMENT_COLORS[entity.element] }
			);
		...
		for (let j = 0; j < vril; j++) {
			sprites.VRIL_4x4.draw(
				ctx,
				scale(480 - 10),
				(i_offset + 1) * scale(67) - (j + superVril) * scale(6) - scale(10),
				scale(4),
				scale(4),
				{ iIndex: ELEMENT_COLORS[entity.element] }
			);
				...
				if (selectedVictims[i].length > 0) {
					...
					if (index < 4) {
						numberText.draw(
							ctx,
							scale(480 - 182 - 8),
							i_offset * scale(67) + scale(49),
							scale(8),
							scale(12),
							{ iIndex: 0, text: selectedVictims[i].length === 1 ? String(index + 1) : "*" }
						);
						sprites.VICTIM_ARROW_8x16.draw(
							ctx,
							scale(480 - 170 - 8),
							i_offset * scale(67) + scale(46),
							scale(8),
							scale(16),
							{ iIndex: 1 }
						);
						...
						numberText.draw(
							...
							scale(8),
							scale(12),
							{ iIndex: 0, text: selectedVictims[i].length === 1 ? String(index - 3) : "*" }
						);
						sprites.VICTIM_ARROW_8x16.draw(
							ctx,
							scale(480 - 182 - 8),
...
function drawCards(battleState, iterator) {
	const {
		turnState: { selectedCards, battleData, selectedVictims },
		playerIndex,
	} = battleState;
	...
	if (playerData === null) {
		...
		if (
			selectedCard !== null &&
			selectedCard !== "PASS" &&
			getSpell(playerData.hand?.[selectedCard]?.id)?.type === SPELL_TYPES.ENCHANTMENT
		) {
			const enchantmentSpell = getSpell(playerData.hand[selectedCard].id);
			let validEnchantees = [];
			if (enchantmentSpell.damage) {
				validEnchantees = [...validEnchantees, SPELL_TYPES.ATTACK_ALL, SPELL_TYPES.ATTACK_BASIC];
			}
			...
			playerData.hand.forEach(({ id, enchantments }, i) => {
				...
				makeInteractable(
					...
					({ x, y, sizeX, sizeY }) => {
						...
						if (enchantments) {
							sprites.ENCHANTMENT_BORDER_48x64.draw(ctx, x, y, sizeX, sizeY, {
								iIndex: iterator % 31,
							});
						}
					},
			...
			playerData.hand.forEach(({ id, enchantments }, i) => {
				const spell = getSpell(id);
				const canSelectCard =
					spell.type === SPELL_TYPES.ENCHANTMENT ||
					getTotalVril(playerData, spell.element) >= spell.vrilRequired;
				if (
					selectedVictims[playerIndex].length === 0 &&
					canSelectCard &&
		...
		if (remainingDeck > 0) {
			ctx.globalAlpha = 1;
			sprites.DECK_REMAIN_48x64.draw(
				ctx,
				scale(240 + (playerData.hand.length - 1) * 25),
				scale(276),
				scale(48),
				scale(64)
			);
			...
			numberText.draw(
				ctx,
				scale(240 + (playerData.hand.length - 1) * 25 + 24 - remainingText.length * 4),
				scale(292),
				scale(8),
				scale(12),
				{ iIndex: 0, text: remainingText }
			);
		}
	}
...
function areAllPlayersReady(turnState) {
	for (let i = 0; i < turnState.battleData.length; i++) {
		...
		if (
			turnState.selectedCards[i] === null ||
			(turnState.selectedCards[i] !== "PASS" && turnState.selectedVictims[i].length === 0)
		) {
			return false;
		}
	}
...
function handleInput(battleState, inputData) {
	...
	if (rightClickPos) {
		if (turnState.selectedCards[playerIndex] !== null && turnState.selectedVictims[playerIndex].length === 0) {
			...
			if (
				getSpell(turnState.battleData[playerIndex]?.hand[turnState.selectedCards[playerIndex]]?.id)?.type ===
				SPELL_TYPES.ATTACK_ALL
			) {
				turnState.selectedCards[playerIndex] = null;
			}
...
function doRound(turnState, victimIndices, postBattleCallback) {
	...
	const calculatedDamages = doesSpellHit
		...
		: ["FAILED"];
	const sequence =
		turnState.battleIndex < 4
			? createLeftAttackSequence(
					turnState,
					turnState.battleIndex,
					victimIndices.toReversed(),
					spell,
					calculatedDamages.toReversed(),
					0
			  )
			: createRightAttackSequence(
					turnState,
					turnState.battleIndex,
					victimIndices.toReversed(),
					spell,
...
function battleGameLoop(timeMs) {
	const {
		battleState,
		battleState: { turnState, onWin, onLose, playerIndex },
	} = state;
	...
	if (turnState.battleIndex === -1) {
		...
		if (areAllPlayersReady(turnState)) {
			state.animationQueue.push(
				new AnimationEngine(
					createWithdrawAnimation(turnState),
					TICK_TIME,
					FPS,
					canvas,
					ctx,
			...
			if (turnState.battleData[playerIndex] === null) {
				...
				(playerIndex < 4 &&
					turnState.battleData
						...
						.filter(({ i }) => i >= 4)
						.filter(({ battleEntity }) => battleEntity !== null).length === 0) ||
				(playerIndex >= 4 &&
					turnState.battleData
						.map((battleEntity, i) => ({ battleEntity, i }))
						.filter(({ i }) => i < 4)
		...
		if (!turnState.battleData[turnState.battleIndex] || turnState.selectedCards[turnState.battleIndex] === "PASS") {
			...
			const victimIndices = turnState.selectedVictims[turnState.battleIndex].filter(
				(i) => turnState.battleData[i] !== null && turnState.battleData[i].entity.health > 0
			);
