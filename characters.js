...
function registerEntity(entity) {
	...
	entitySpriteDirectory[`${entity.spritePath}.idle`] = new Sprite(
		`./img/characters/${entity.spritePath}/idle_64x64.png`,
		64,
		64,
		entity.idleFrames
	);
	entitySpriteDirectory[`${entity.spritePath}.cast`] = new Sprite(
		`./img/characters/${entity.spritePath}/cast_64x64.png`,
		64,
		64,
		entity.castFrames
	);
	entitySpriteDirectory[`${entity.spritePath}.death`] = new Sprite(
		`./img/characters/${entity.spritePath}/death_64x64.png`,
		64,
		64,
		entity.deathFrames
	);
...
MP_SPRITES.forEach(({ spritePath, idleFrames, castFrames, deathFrames }) => {
	entitySpriteDirectory[`${spritePath}.idle`] = new Sprite(
		`./img/characters/${spritePath}/idle_64x64.png`,
		64,
		64,
		idleFrames
	);
	entitySpriteDirectory[`${spritePath}.cast`] = new Sprite(
		`./img/characters/${spritePath}/cast_64x64.png`,
		64,
		64,
		castFrames
	);
	entitySpriteDirectory[`${spritePath}.death`] = new Sprite(
		`./img/characters/${spritePath}/death_64x64.png`,
		64,
		64,
		deathFrames
	);
...
const wizardIds = ["air_wizard", "fire_wizard", "water_wizard", "earth_wizard"];

const level1CreatureIds = ["earth_skeleton_lv1", "fire_skeleton_lv1", "air_skeleton_lv1", "water_skeleton_lv1"];

const level2CreatureIds = ["earth_skeleton_lv2", "water_ghost_lv2", "air_ghost_lv2"];

const level3CreatureIds = ["earth_goomborb_lv3", "fire_phoenix_lv3"];
...
const randomAI = (index, battleData) => {
	...
	const options = accessibleCards
		...
		.map(({ d, i }) => ({
			selectedCard: i,
			options: d.map(({ selectedVictim }) => selectedVictim),
		}))
	...
	if (getSpell(entityData.hand[selectedCardOptions.selectedCard].id).type === SPELL_TYPES.ATTACK_ALL) {
		selectedVictims = battleData
			.map((e, i) => ({ e, i }))
			.filter(
				({ e, i }) =>
					e !== null &&
					canUseSpellOn(getSpell(entityData.hand[selectedCardOptions.selectedCard].id), index, i)
			)
			.map(({ i }) => i);
	} else {
