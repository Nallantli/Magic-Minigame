const MP_ELEMENTS = ["air", "fire", "water", "earth"];
		...
		switch (data.action) {
			...
			case "BATTLE_ANIMATION_DATA": {
				state.animationQueue.push(
					new AnimationEngine(
						createWithdrawAnimation(state.battleState.turnState),
						TICK_TIME,
						FPS,
						canvas,
						ctx,
						() => {
							...
							const finalAnimation = () => {
								if (state.battleState.win) {
									...
									state.animationQueue.push(
										new AnimationEngine(
											getReturnSequence(finalTurnState),
											TICK_TIME,
											FPS,
											canvas,
											ctx,
...
function menuGameLoop(timeMs) {
	...
	if (state.menuState.errorMessage) {
		font.draw(ctx, scale(240 - state.menuState.errorMessage.length * 3), scale(340), scale(6), scale(8), {
			iIndex: 2,
			text: state.menuState.errorMessage,
		});
	}
	...
	makeTextBox(
		...
		({ x, y, sizeX, sizeY, isFocused, value }) => {
			if (!isFocused && value === "") {
				font.draw(ctx, x + scale(12), y - scale(4), scale(3), scale(4), { iIndex: 0, text: "Player Name:" });
			}
			if (isFocused) {
			...
			font.draw(ctx, x + scale(3), y + scale(3), scale(12), scale(16), { iIndex: 0, text: value });
		},
		(value, key) => {
			switch (key) {
				case "backspace":
	...
	makeTextBox(
		...
		({ x, y, sizeX, sizeY, isFocused, value }) => {
			if (!isFocused && value === "") {
				font.draw(ctx, x + scale(12), y - scale(4), scale(3), scale(4), { iIndex: 0, text: "Room Code:" });
			}
			if (isFocused) {
				ctx.fillStyle = COLORS_HEX.white;
				ctx.fillRect(x - scale(2), y - scale(2), sizeX + scale(4), sizeY + scale(4));
		...
		(value, key) => {
			switch (key) {
				...
				default:
					...
					if (
						key.length === 1 &&
						((key.charCodeAt(0) >= "a".charCodeAt(0) && key.charCodeAt(0) <= "z".charCodeAt(0)) ||
	...
	MP_ELEMENTS.forEach((element, i) => {
		makeInteractable(
			scale(240 - MP_ELEMENTS.length * 20 + i * 40 + 6),
			scale(60),
			scale(32),
			scale(32),
			({ x, y, sizeX, sizeY }) => {
	...
	MP_SPRITES.forEach((spriteData, i) => {
		if (wizardSelectionIndex === i) {
			ctx.globalAlpha = 1;
			entitySpriteDirectory[`${spriteData.spritePath}.idle`].draw(
				ctx,
