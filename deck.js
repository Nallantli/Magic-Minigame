...
function deckGameLoop(timeMs) {
	...
	sprites.ELEMENTS_MINOR_8x8.draw(ctx, scale(17), scale(36), scale(8), scale(8), {
		iIndex: ELEMENT_COLORS[currentEntity.element],
	});
	font.draw(ctx, scale(27), scale(36), scale(6), scale(8), { iIndex: 0, text: currentEntity.name });
	font.draw(ctx, scale(33 + currentEntity.name.length * 6), scale(36), scale(6), scale(8), {
		iIndex: ELEMENT_COLORS[currentEntity.element],
		text: `[${currentEntity.health}/${currentEntity.maxHealth}]`,
	});
		...
		for (let j = 0; j < 5; j++) {
			if (deckIndex < deck.length) {
				...
				makeInteractable(
					...
					scale(48),
					({ x, y, sizeX, sizeY }) => {
						getCardSprite(card).draw(ctx, x, y, sizeX, sizeY, {
							cropY: 48,
						});
					},
					({ x, y, sizeX, sizeY, renderCallback }) => {
						...
						toolTips.push(
							makeToolTip(scale(78), scale(114), ({ x, y }) => {
								getCardSprite(card).draw(ctx, x + scale(3), y + scale(3), scale(72), scale(96));
								font.draw(ctx, x + scale(4), y + scale(102), scale(6), scale(8), {
									iIndex: ELEMENT_COLORS[card.element],
									text: card.name,
								});
							})
						);
			...
			sprites.DECK_SLOT_50x50.draw(ctx, scale(14 + j * 50), scale(49 + i * 50), scale(50), scale(50), {
				iIndex: deckIndex < deck.length,
			});
			deckIndex++;
		}
	}
	...
	makeInteractable(
		...
		scale(32),
		({ x, y, sizeX, sizeY }) =>
			sprites.BACK_32x32.draw(ctx, scale(480 - 32), scale(375 - 32), scale(32), scale(32)),
		({ x, y, sizeX, sizeY, renderCallback }) => {
			renderCallback();
