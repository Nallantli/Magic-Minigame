function entitySelectionGameLoop(timeMs) {
	...
	if (selectedEntity) {
		getIdleSprite(selectedEntity).draw(ctx, scale(280), scale(16), scale(128), scale(128), {
			iIndex: iterator % getIdleSprite(selectedEntity).indices,
		});
		ELEMENT_ICONS[selectedEntity.element].draw(ctx, scale(328), scale(152), scale(32), scale(32));
		font.draw(ctx, scale(344 - selectedEntity.name.length * 3), scale(186), scale(6), scale(8), {
			iIndex: 0,
		...
		statStrings.forEach((str, i) =>
			font.draw(ctx, scale(286), scale(206 + i * 8), scale(6), scale(8), { iIndex: 0, text: str })
		);
	}
