...
function mapGameLoop(timeMs) {
	if (state.mapState === undefined) {
		const { rooms, map, crossings } = generateDungeon(30, 30, 5, 5, 10, 10);
		let enemyRooms = rooms.filter(({ x, y, sizeX, sizeY }) => sizeX * sizeY > 15 && !(x === 0 && y === 0));
		...
		for (let i = 0; i < 5; i++) {
			...
			if (room.sizeX * room.sizeY > 40) {
				enemies.push({
					...
					roomId: room.id,
					model: {
						...randomFromList(entityDirectory.creatures),
		...
		let freeRooms = rooms.filter(room => !roomWithEnemiesIds.includes(room.id));
		...
		state.mapState = {
			...
			tileSize: 32,
			entities: [
				{ id: 'player_character', x: 2, y: 2, mirror: false },
				...enemies
			],
			items
		};
	...
	if (entitiesNear.length > 0) {
		...
		state = {
			...state,
			path: 'BATTLE',
			battleState: generateBattleState([state.player], entitiesInRoom,
				() => {
					battleTrack.pause();
